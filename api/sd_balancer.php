 ...
 * api/sd_balancer.php
 *
 * Load-balancing logic for AUTOMATIC1111 / Stable Diffusion endpoints.
 *
 * Selection rules (applied in order):
 *   1. Only active sd_endpoints.
 *   2. Only endpoints with fewer than 4 currently-running sd_tasks.
 ...
 */
function pickSdEndpoint(string $mode = 'txt2img'): ?array
{
    $db = getDb();
    $db->beginTransaction();
    try {
 ...
 */
function completeSdTask(int $taskId, string $status = 'done'): void
{
    getDb()->prepare('
