 ...
 * api/sd_balancer.php
 *
 * Load-balancing / routing engine for AUTOMATIC1111 / Stable Diffusion endpoints.
 * Shares its circuit-breaker, backoff, orphan-cleanup and scoring logic with
 * api/balancer.php and api/comfy_balancer.php via lib/balancer_engine.php.
 *
...
/**
 * Pick the best available SD endpoint, register a new task, and return an
 * associative array: ['endpoint' => <row>, 'task_id' => int]
 *
 * Returns null when no endpoint with available capacity exists.
 *
 * @param string   $mode          'txt2img' or 'img2img'
 ...
 */
function pickSdEndpoint(string $mode = 'txt2img', ?int $maxConcurrent = null): ?array
{
    cleanupOrphanedTasks('sd_tasks');
    ...
    try {
        $stmt = $db->prepare("
            SELECT e.id, e.base_url, e.timeout,
        ...
        foreach ($candidates as $endpoint) {
            ...
            $lockStmt = $db->prepare('SELECT id, is_active FROM sd_endpoints WHERE id = ? FOR UPDATE');
            ...
            $countStmt = $db->prepare("SELECT COUNT(*) FROM sd_tasks WHERE endpoint_id = ? AND status = 'running'");
            ...
            maybeHalfOpenCircuit('sd_endpoints', $endpointId);

            $db->prepare(
                "INSERT INTO sd_tasks (endpoint_id, mode, status) VALUES (?, ?, 'running')"
            )->execute([$endpointId, $mode]);
...
/**
 * Mark an SD task as finished.
 *
 * @param int        $taskId    Primary key of the sd_tasks row.
 * @param string     $status    'done' or 'error'.
 * @param float|null $latencyMs Observed request latency in milliseconds.
 */
function completeSdTask(int $taskId, string $status = 'done', ?float $latencyMs = null): void
{
    $db = getDb();
    $db->prepare('
        UPDATE sd_tasks SET status = ?, finished_at = NOW(3) WHERE id = ?
    ')->execute([$status, $taskId]);

    try {
        $epStmt = $db->prepare('SELECT endpoint_id FROM sd_tasks WHERE id = ?');
        $epStmt->execute([$taskId]);
        $endpointId = (int) $epStmt->fetchColumn();
        if ($endpointId > 0) {
            recordEndpointOutcome('sd_endpoints', $endpointId, $status === 'done', $latencyMs);
        }
    } catch (Throwable $_e) {
